package sflow

import (
	"bytes"
	"testing"

	"github.com/netsampler/goflow2/v2/decoders/utils"
	"github.com/stretchr/testify/assert"
)

func TestSFlowDecode(t *testing.T) {
	data := []byte{
		,
	}
	buf := bytes.NewBuffer(data)
	var packet Packet
	assert.NoError(t, DecodeMessageVersion(buf, &packet))
}

func TestExpandedSFlowDecode(t *testing.T) {
	data := []byte{
		,
	}

	buf := bytes.NewBuffer(data)
	var packet Packet
	assert.NoError(t, DecodeMessageVersion(buf, &packet))
}

func TestSFlowDecodeDropEgressQueue(t *testing.T) {
	data := []byte{
		0x00, 0x00, 0x00, 0x05, 0x00, 0x00, 0x00, 0x01, 0xc0, 0xa8, 0x77, 0xb8, 0x00, 0x01, 0x86, 0xa0,
		0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x30, 0x7e, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x05,
		0x00, 0x00, 0x00, 0x2C, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01,
		0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x01,
		0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x04, 0x0c, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x2a,
	}

	buf := bytes.NewBuffer(data)
	var packet Packet
	assert.NoError(t, DecodeMessageVersion(buf, &packet))
	assert.Len(t, packet.Samples, 1)
	assert.NotNil(t, packet.Samples[0])
	sample, ok := packet.Samples[0].(DropSample)
	assert.True(t, ok)
	assert.Len(t, sample.Records, 1)
	assert.Equal(t, EgressQueue{Queue: 42}, sample.Records[0].Data)
}

func TestSFlowDecodeDropExtendedACL(t *testing.T) {
	data := []byte{
		0x00, 0x00, 0x00, 0x05, 0x00, 0x00, 0x00, 0x01, 0xc0, 0xa8, 0x77, 0xb8, 0x00, 0x01, 0x86, 0xa0,
		0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x30, 0x7e, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x05,
		0x00, 0x00, 0x00, 0x38, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01,
		0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x01,
		0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x04, 0x0d, 0x00, 0x00, 0x00, 0x10, 0x00, 0x00, 0x00, 0x2a,
		0x00, 0x00, 0x00, 0x03, 0x66, 0x6f, 0x6f, 0x99, 0x00, 0x00, 0x00, 0x02,
	}

	buf := bytes.NewBuffer(data)
	var packet Packet
	assert.NoError(t, DecodeMessageVersion(buf, &packet))
	assert.Len(t, packet.Samples, 1)
	assert.NotNil(t, packet.Samples[0])
	sample, ok := packet.Samples[0].(DropSample)
	assert.True(t, ok)
	assert.Len(t, sample.Records, 1)
	assert.Equal(t, ExtendedACL{Number: 42, Name: "foo", Direction: 2}, sample.Records[0].Data)
}

func TestSFlowDecodeDropExtendedFunction(t *testing.T) {
	data := []byte{
		0x00, 0x00, 0x00, 0x05, 0x00, 0x00, 0x00, 0x01, 0xc0, 0xa8, 0x77, 0xb8, 0x00, 0x01, 0x86, 0xa0,
		0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x30, 0x7e, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x05,
		0x00, 0x00, 0x00, 0x32, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01,
		0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x01,
		0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x04, 0x0e, 0x00, 0x00, 0x00, 0x0a, 0x00, 0x00, 0x00, 0x06,
		0x66, 0x6f, 0x6f, 0x62, 0x61, 0x72,
	}

	buf := bytes.NewBuffer(data)
	var packet Packet
	assert.NoError(t, DecodeMessageVersion(buf, &packet))
	assert.Len(t, packet.Samples, 1)
	assert.NotNil(t, packet.Samples[0])
	sample, ok := packet.Samples[0].(DropSample)
	assert.True(t, ok)
	assert.Len(t, sample.Records, 1)
	assert.Equal(t, ExtendedFunction{Symbol: "foobar"}, sample.Records[0].Data)
}

func TestSFlowDecodeHostDescr(t *testing.T) {
	data := []byte{
		0x00, 0x00, 0x00, 0x05, 0x00, 0x00, 0x00, 0x01, 0xc0, 0xa8, 0x01, 0x01, 0x00, 0x00, 0x00, 0x01,
		0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x64, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x02,
		0x00, 0x00, 0x00, 0x44, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01,
		0x00, 0x00, 0x07, 0xd0, 0x00, 0x00, 0x00, 0x30, 0x00, 0x00, 0x00, 0x08, 0x74, 0x65, 0x73, 0x74,
		0x68, 0x6f, 0x73, 0x74, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c,
		0x0d, 0x0e, 0x0f, 0x10, 0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x06,
		0x35, 0x2e, 0x31, 0x30, 0x2e, 0x30, 0x00, 0x00,
	}

	buf := bytes.NewBuffer(data)
	var packet Packet
	assert.NoError(t, DecodeMessageVersion(buf, &packet))
	assert.Len(t, packet.Samples, 1)

	sample, ok := packet.Samples[0].(CounterSample)
	assert.True(t, ok)
	assert.Len(t, sample.Records, 1)

	hostDescr, ok := sample.Records[0].Data.(HostDescr)
	assert.True(t, ok)
	assert.Equal(t, "testhost", hostDescr.Hostname)
	expectedUUID := utils.UUID{0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10}
	assert.Equal(t, expectedUUID, hostDescr.UUID)
	assert.Equal(t, uint32(3), hostDescr.MachineType)
	assert.Equal(t, uint32(2), hostDescr.OSName)
	assert.Equal(t, "5.10.0", hostDescr.OSRelease)
}

func TestSFlowDecodeHostCPU(t *testing.T) {
	data := []byte{
		0x00, 0x00, 0x00, 0x05, 0x00, 0x00, 0x00, 0x01, 0xc0, 0xa8, 0x01, 0x01, 0x00, 0x00, 0x00, 0x01,
		0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0xc8, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x02,
		0x00, 0x00, 0x00, 0x58, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01,
		0x00, 0x00, 0x07, 0xd3, 0x00, 0x00, 0x00, 0x44, 0x3f, 0xc0, 0x00, 0x00, 0x40, 0x00, 0x00, 0x00,
		0xbf, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x96, 0x00, 0x00, 0x00, 0x04,
		0x00, 0x00, 0x09, 0x60, 0x00, 0x01, 0x51, 0x80, 0x00, 0x00, 0x27, 0x10, 0x00, 0x00, 0x00, 0x64,
		0x00, 0x00, 0x13, 0x88, 0x00, 0x01, 0x11, 0x70, 0x00, 0x00, 0x03, 0xe8, 0x00, 0x00, 0x00, 0x32,
		0x00, 0x00, 0x00, 0x14, 0x00, 0x00, 0xc3, 0x50, 0x00, 0x01, 0x86, 0xa0,
	}

	buf := bytes.NewBuffer(data)
	var packet Packet
	assert.NoError(t, DecodeMessageVersion(buf, &packet))

	sample, ok := packet.Samples[0].(CounterSample)
	assert.True(t, ok)

	hostCPU, ok := sample.Records[0].Data.(HostCPU)
	assert.True(t, ok)
	assert.InDelta(t, 1.5, hostCPU.LoadOne, 0.01)
	assert.InDelta(t, 2.0, hostCPU.LoadFive, 0.01)
	assert.InDelta(t, -1.0, hostCPU.LoadFifteen, 0.01)
	assert.Equal(t, uint32(2), hostCPU.ProcRun)
	assert.Equal(t, uint32(150), hostCPU.ProcTotal)
	assert.Equal(t, uint32(4), hostCPU.CPUNum)
	assert.Equal(t, uint32(2400), hostCPU.CPUSpeed)
	assert.Equal(t, uint32(86400), hostCPU.Uptime)
}

func TestSFlowDecodeHostMemory(t *testing.T) {
	data := []byte{
		,
	}

	buf := bytes.NewBuffer(data)
	var packet Packet
	assert.NoError(t, DecodeMessageVersion(buf, &packet))

	sample, ok := packet.Samples[0].(CounterSample)
	assert.True(t, ok)

	hostMemory, ok := sample.Records[0].Data.(HostMemory)
	assert.True(t, ok)
	assert.Equal(t, uint64(0x400000000), hostMemory.MemTotal)
	assert.Equal(t, uint64(0x200000000), hostMemory.MemFree)
	assert.Equal(t, uint32(1000), hostMemory.PageIn)
	assert.Equal(t, uint32(500), hostMemory.PageOut)
}

func TestSFlowDecodeExtendedSocketIPv4(t *testing.T) {
	data := []byte{
		0x00, 0x00, 0x00, 0x05, 0x00, 0x00, 0x00, 0x01, 0xc0, 0xa8, 0x01, 0x01, 0x00, 0x00, 0x00, 0x01,
		0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x01, 0x90, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01,
		0x00, 0x00, 0x00, 0x3c, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x64,
		0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x08, 0x34, 0x00, 0x00, 0x00, 0x14, 0x00, 0x00, 0x00, 0x06,
		0xc0, 0xa8, 0x01, 0x64, 0x0a, 0x00, 0x00, 0x32, 0x00, 0x00, 0x1f, 0x90, 0x00, 0x00, 0x01, 0xbb,
	}

	buf := bytes.NewBuffer(data)
	var packet Packet
	assert.NoError(t, DecodeMessageVersion(buf, &packet))

	sample, ok := packet.Samples[0].(FlowSample)
	assert.True(t, ok)

	socket, ok := sample.Records[0].Data.(ExtendedSocketIPv4)
	assert.True(t, ok)
	assert.Equal(t, uint32(6), socket.Protocol)
	assert.Equal(t, []byte{192, 168, 1, 100}, []byte(socket.LocalIP))
	assert.Equal(t, []byte{10, 0, 0, 50}, []byte(socket.RemoteIP))
	assert.Equal(t, uint32(8080), socket.LocalPort)
	assert.Equal(t, uint32(443), socket.RemotePort)
}
